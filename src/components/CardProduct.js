  ...
  if (props.product) {
    ...
    return (
      <Card className="shadow border-0">
        <Card.Img
          variant="top"
          src={props.cartData.image}
          style={{ objectFit: "cover", height: "13em" }}
          alt="thumbnail"
        />
        <Card.Body>
          <Card.Title>
            <strong>{props.cartData.name}</strong>
          </Card.Title>
          <Card.Title className="text-muted" style={{ fontSize: "16px" }}>
            {props.cartData.seller}
          </Card.Title>
          <Card.Title className="text-dark" style={{ fontSize: "14px" }}>
            Quantity: {props.cartData.quantity}
          </Card.Title>
          {props.cartData.prices.isDiscount ? (
            <div className="d-flex">
              <Card.Text
                className="me-2"
                style={{
              ...
              >
                Rp. {numberWithCommas(props.cartData.prices.actualPrice)}
              </Card.Text>
              <Card.Text>
                <strong>
                  Rp. {numberWithCommas(props.cartData.prices.discountPrice)}
                </strong>
              </Card.Text>
            </div>
          ) : (
